...
namespace Icemix\SocialGrant\Grants;
...
class SocialGrant extends AbstractGrant
{
    ...
    public function validateUser(ServerRequestInterface $request): UserEntity
    {
        ...
        if($provider == "twitter"){
            ...
            if (is_null($secret)) {
                throw OAuthServerException::invalidRequest('secret');
            }
            $user = $this->resolver->resolveUserByProviderAccessTokenAndSecret($provider, $accessToken,$secret);
        }else{
